...
async def user_flow_guardian(
    ...
    del sitio web según su rol. Versión final y robusta.
    """
    ...
    # 1. Usuario recién autenticado -> DEBE ir a la página de onboarding.
    ...
    # 2. Usuario Cliente -> DEBE estar en la ZONA de cliente.
    elif current_user.role in client_roles:
        client_zone_prefix = "/dashboard/client"
        if not current_path.startswith(client_zone_prefix):
            # Excepción: un cliente PUEDE necesitar volver a ver la página de onboarding
            # o alguna otra página común. Si es así, no lo redirijas.
            # Por ahora, si no está en su zona, lo mandamos a la raíz de su zona.
    ...
    # 3. Usuario Staff/Admin -> DEBE estar en la ZONA de staff.
