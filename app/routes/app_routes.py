...
# En app/routes/app_routes.py, añade esta función al router:

@router.get("/payments/new-form", response_class=HTMLResponse)
async def get_payment_form(
    request: Request,
    current_user: User = Depends(get_current_active_user)
):
    """
    Este endpoint devuelve ÚNICAMENTE el HTML del formulario de pago.
    Está diseñado para ser llamado por HTMX y cargado en un modal.
    """
    # ¡RUTA CORREGIDA! Apunta a la nueva ubicación del partial.
    return templates.TemplateResponse(
        "payments/partials/_payment_form.html", 
        {"request": request}
    )